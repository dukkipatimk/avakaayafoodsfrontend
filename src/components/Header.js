...
const Header = () => {
  ...
  // The admin screens are a back office, not a shopfront: the shopping
  // furniture (floating chat/cart, mobile shopping toolbar) is left off them.
  ...
  return (
    <>
      ...
      <header className={`header ${scrolled ? 'header--scrolled' : ''}`}>
        <div className="header-inner container-wide">
          ...
          <div className="header-actions">
            ...
            {/* No cart in the header. The cart already has two permanent homes
                on screen — the floating side button, and the pill that appears
                the moment there is something in it — and a third copy up here
                was the one furthest from where people are looking. */}
          </div>
        </div>
      </header>
      ...
      {/* Mobile bottom toolbar — shopping only. Home, search, quick order and
          account are of no use to someone working the orders list, and the bar
          covers the last row of every admin table. */}
      {!isAdminArea && (
      ...
      {/* Right-side floating action menu (desktop only) — the shop's chat and
          cart buttons. On admin they sat on top of the table's right-hand
          column and offered to sell the shop its own pickles. */}
      {!isAdminArea && (
      <div className="side-menu" aria-label="Quick actions">
        {/* No floating WhatsApp button: the number is already a contact pill
            in the header, and in the footer besides. */}
